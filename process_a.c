...
void *send_thread(void *shared_st) {
    ...
    while (shared_stuff->sh_running) {
        ...
        fgets(shared_stuff->point_to_local_bufferA, BUFSIZ/2, stdin);
...
void *receive_thread(void *shared_st) {
    ...
    while (shared_stuff->sh_running) {
        ...
        if (shared_stuff->constracting_msg_A) {
...
{   
    ...
    if (shared_stuff->sh_running == 0 && shared_stuff->procB_exited == 1) {
        printf("procB exited\n");
